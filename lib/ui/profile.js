...
function getAvatar(userObj, callback) {
    ...
    ImageToAscii(userObj.avatar_url, {
        size: { height: "70%" }
      , colored: Config.coloredImages
...
module.exports = function (options, progress, callback) {
    Profile(options, progress, function (err, data) {
        ...
        getAvatar(data, function (err, cAvatar) {
            ...
            info.push(separator);
            if (data.type === "User") {
                info.push("Followe(R)s: " + data.followers);
                info.push("Followi(N)g: " + data.following);
            } else if (data.type === "Organization") {
                info.push("Press SHIFT + M to see the organization members.");
            }
            ...
            var cRepos = repos.stringify().trim();
